...
router.get("/state", (req, res) => {
  proxyGet("/gopro/camera/state", req, res);
});

// GET /shutter/:action -> /gopro/camera/shutter/{start|stop}
router.get("/shutter/:action", (req, res) => {
  const action = req.params.action;
  if (action !== "start" && action !== "stop") {
    res.status(400).json({ error: "Invalid action. Use 'start' or 'stop'." });
    return;
  }
  proxyGet(`/gopro/camera/shutter/${action}`, req, res);
});

// GET /presets/set_group?id=N -> /gopro/camera/presets/set_group?id=N
router.get("/presets/set_group", (req, res) => {
  const id = req.query.id;
  if (id === undefined) {
    res.status(400).json({ error: "Missing required query parameter 'id'." });
    return;
  }
  proxyGet(`/gopro/camera/presets/set_group?id=${id}`, req, res);
});
...
router.get("/stream/:action", (req, res) => {
  ...
  proxyGet(`/gopro/camera/stream/${action}`, req, res);
});

// GET /keep_alive -> /gopro/camera/keep_alive
router.get("/keep_alive", (req, res) => {
  proxyGet("/gopro/camera/keep_alive", req, res);
});
