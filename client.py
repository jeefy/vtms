...
try:
    ...
    GPS_AVAILABLE = False
    logging.warning(
        "pynmea2 or pyserial not available - GPS functionality will be disabled"
    )
...
class VTMSClient:
    ...
    def setup_mqtt(self) -> bool:
        ...
        try:
            ...
            self.mqttc.connect(
                config.config.mqtt_server,
                config.config.mqtt_port,
                config.config.mqtt_keepalive,
            )
            # Don't start loop here - we'll handle it in the run method
            logger.info(f"MQTT client connected to {config.config.mqtt_server}")
            return True
    ...
    async def setup_gps_connection(self) -> bool:
        ...
        if not GPS_AVAILABLE:
            logger.warning(
                "GPS functionality disabled - pynmea2 or pyserial not available"
            )
            return False
    ...
    async def start_gps_monitoring(self):
        ...
        if not GPS_AVAILABLE:
            logger.warning(
                "GPS monitoring disabled - pynmea2 or pyserial not available"
            )
        ...
        while True:
            try:
                ...
                if (
                    last_gps_data["latitude"] is not None
                    and last_gps_data["longitude"] is not None
                ):
                    ...
                    if config.config.debug:
                        logger.debug(
                            f"GPS data published: {published_count} topics, "
    ...
    def _publish_message(self, topic: str, payload, qos: int = 0, retain: bool = False):
        ...
        try:
            if self.mqtt_connected and self.mqttc:
                ...
                if result.rc != 0:
                    logger.warning(
                        f"MQTT publish failed with return code {result.rc}, buffering message"
                    )
    ...
    def _buffer_message(self, topic: str, payload, qos: int = 0, retain: bool = False):
        ...
        try:
            ...
            while len(self.message_buffer) >= self.max_buffer_size:
                ...
                logger.warning(
                    f"Dropping old buffered message to topic {old_msg['topic']}"
                )
    ...
    def _flush_message_buffer(self):
        ...
        while self.message_buffer:
            ...
            if current_time - message["timestamp"] > 300:
                logger.warning(
                    f"Dropping expired buffered message to {message['topic']}"
                )
                continue
    ...
    def _on_connect(self, client, userdata, flags, reason_code, properties):
        ...
        try:
            if reason_code == 0:
                ...
                self.mqtt_retry_count = 0
                logger.info(
                    f"MQTT connected successfully with result code {reason_code}"
                )
    ...
    def _process_obd_response(self, command: str, response):
        ...
        if command in myobd.metric_commands:
            ...
            if config.config.debug:
                logger.warning(
                    f'No proper handler for query type "{command}" -- defaulting to Metric'
                )
            myobd.new_metric(response, mqttc=self.mqtt_wrapper)
    ...
    async def setup_obd_connection(self) -> bool:
        ...
        while True:
            ...
            if len(ports) == 0:
                logger.warning(
                    "No OBDII ports found. Sleeping for 15s then retrying..."
                )
            ...
            for port in ports:
                self.obd_connection = obd.Async(
                    port, timeout=5, delay_cmds=0, start_low_power=True, fast=True
                )
                if self.obd_connection.status() is OBDStatus.CAR_CONNECTED:
                    logger.info(f"Connected to OBDII port on {port}")
                    return True
                logger.warning(f"No connection to OBDII port from {port}")

            # All ports scanned, none connected — retry after delay
            logger.warning("No OBDII connection on any port. Retrying...")
            await asyncio.sleep(config.config.obd_retry_delay)
    ...
    def setup_obd_watches(self):
        ...
        # Set up DTC watch
        self.obd_connection.watch(
            obd.commands.GET_DTC, callback=partial(myobd.new_dtc, mqttc=self.mqttc)
        )
    ...
    async def _run_obd_monitoring(self):
        ...
        try:
            ...
            while True:
                ...
                if (
                    self.obd_connection
                    and self.obd_connection.status() != OBDStatus.CAR_CONNECTED
                ):
        ...
        while True:
            try:
                ...
                if not self.mqtt_connected:
                    ...
                    logger.warning(
                        f"MQTT disconnected, attempting reconnection #{self.mqtt_retry_count}"
                    )
                    ...
                    try:
                        ...
                        if self.mqtt_connected:
                            logger.info(
                                f"MQTT reconnected successfully after {self.mqtt_retry_count} attempts"
                            )
                        else:
        ...
        while True:
            try:
                ...
                if mqtt_connected:
                    ...
                    logger.debug(
                        f"Health status published: MQTT={mqtt_connected}, OBD={obd_connected}"
                    )
