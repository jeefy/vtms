...
# The wiring module has not been implemented yet; skip the entire file.
wiring = pytest.importorskip("wiring", reason="wiring package not yet implemented")
from wiring.parser import Wire, Pin, parse_wiring_file  # noqa: E402
from wiring.diagram import generate_svg, generate_pinouts  # noqa: E402
...
def sample_wires():
    ...
    return [
        Wire(
            color="RED",
            source=Pin("Fuse Box", "3"),
            destination=Pin("Start Button", "1"),
            gauge="18",
        ),
        Wire(
            color="BLK",
            source=Pin("Start Button", "2"),
            destination=Pin("Chassis", "GND1"),
            gauge="18",
        ),
        Wire(
            color="GRN",
            source=Pin("ECU", "1"),
            destination=Pin("Coolant Temp Sensor", "1"),
            gauge="22",
        ),
    ]
