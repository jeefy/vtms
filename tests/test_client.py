...
class TestVTMSClient:
    ...
    @patch.object(Config, "is_raspberrypi")
    @patch("client.logger")
    ...
    def test_setup_message_handlers(self):
        ...
        with patch.object(Config, "is_raspberrypi", return_value=False):
            client = VTMSClient()
    ...
    def test_setup_mqtt_success(self, mock_logger, mock_config, mock_mqtt_class):
        ...
        mock_config.mqtt_server = "test.mqtt.server"
        ...
        with patch.object(Config, "is_raspberrypi", return_value=False):
            ...
            mock_logger.info.assert_called_with(
                "MQTT client connected to test.mqtt.server"
            )
    ...
    def test_setup_mqtt_failure(self, mock_logger, mock_mqtt_class):
        ...
        with patch.object(Config, "is_raspberrypi", return_value=False):
            ...
            mock_logger.error.assert_called_with(
                "Failed to setup MQTT: Connection failed"
            )
    ...
    @patch("src.config.config")
    @patch("client.logger")
    async def test_start_gps_monitoring_success(
        self, mock_logger, mock_config, mock_get_current, mock_connect
    ):
    ...
    @patch("gpsd2.get_current")
    @patch("client.logger")
    async def test_start_gps_monitoring_error(
        self, mock_logger, mock_get_current, mock_connect
    ):
    ...
    @patch("src.config.config")
    @patch("client.logger")
    def test_on_message_callback(self, mock_logger, mock_config):
    ...
    def test_setup_obd_connection_success(self, mock_logger, mock_obd_async, mock_scan):
        ...
        with patch.object(Config, "is_raspberrypi", return_value=False):
            ...
            mock_logger.info.assert_called_with(
                "Connected to OBDII port on /dev/ttyUSB0"
            )
    ...
    def test_setup_obd_watches(self, mock_logger, mock_commands):
        ...
        with patch.object(Config, "is_raspberrypi", return_value=False):
            ...
            assert (
                len(client.obd_connection.watched_commands) >= 3
            )  # metrics + monitor + DTC
...
class TestMQTTBuffering:
    ...
    def test_publish_message_when_connected(self, mock_logger, mock_is_pi):
        ...
        assert result is True
        client.mqttc.publish.assert_called_once_with(
            "test/topic", '{"key": "value"}', 0, False
        )
    ...
    def test_flush_message_buffer_expired_messages(self, mock_logger, mock_is_pi):
        ...
        client.mqttc.publish.assert_called_once_with(
            "test/fresh", "fresh message", 0, False
        )
        assert len(client.message_buffer) == 0
...
class TestMQTTWrapper:
    ...
    def test_mqtt_wrapper_publish(self, mock_is_pi):
        ...
        assert result is True
        client._publish_message.assert_called_once_with(
            "test/topic", "test message", 1, True
        )
